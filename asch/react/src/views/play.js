...
import { withLocation } from '../components/with_location';
    ...
    return (
        <div className="h-screen w-screen grid justify-items-center">
            <div className="game m-auto" id="gameWindow">
                ...
                />
                <div>
                    Note: This may take some time to load depending on your internet and computer speed. Thank you for
                    your patience.
                </div>
            </div>
        </div>
    ...
    componentDidMount() {
        ...
        fetch(request).then((response) => {
            if (response.status !== 200) {
                toast.error(
                    'Error fetching data from server. Try refreshing the page, or checking the link for correctness.',
                    {
                        position: 'top-right',
                        autoClose: 5000,
                        hideProgressBar: false,
...
export default withLocation(PlayView);
